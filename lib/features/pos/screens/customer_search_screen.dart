...
class CustomerSearchScreen extends StatefulWidget {
  final bool isCoupon;
  final bool fromOrderFilter;
  const CustomerSearchScreen({super.key, this.isCoupon = false, this.fromOrderFilter = false});
            ...
            builder: (context, couponProvider, _) {
              return Consumer<CustomerController>(
                  builder: (context, customerProvider, child) {
                    ...
                    if(widget.isCoupon) {
                      customerList.addAll(couponProvider.couponCustomerList ?? []);
                    } else {
                      customerList.addAll(customerProvider.searchedCustomerList ?? []);
                    }
                          ...
                          return InkWell(
                            ...
                            onTap: () {
                              if(widget.isCoupon) {
                                ...
                                Provider.of<CouponController>(context, listen: false).setCouponCustomerIndex(index, customerList[index].id ?? 0, true);
                              } else {
